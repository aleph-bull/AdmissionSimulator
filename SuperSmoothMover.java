...
public abstract class SuperSmoothMover extends Actor
{
    ...
    /**
     * Returns the distance from Actor to coordinates
     */
    public double getDistance(double x, double y) {
        return Math.sqrt(Math.pow(exactX - x, 2) + Math.pow(exactY - y, 2));
    }
    
    /**
     * Returns the distance from a Actor to other Actor
     */
    public double getDistance(Actor other) {
        if(other == null) {
            return 9999999; // if the object is null, return a large number to make this not preffered in methods 
        }
        if(other instanceof SuperSmoothMover) {
            SuperSmoothMover s = (SuperSmoothMover) other; // if the actor is a SuperSmoothMover, return the double coordinates
            return getDistance(s.getPreciseX(), s.getPreciseY());
        }
        return getDistance(other.getX(), other.getY());
    }
    
    public void moveToward(double speed, double x, double y){
        ...
        setLocation(exactX + moveX, exactY + moveY);
    }
    
    public void moveToward(double speed, SuperSmoothMover other){
        if(other == null) {
            return;
        }
        moveToward(speed, other.getPreciseX(), other.getPreciseY());
    }
